...
public class Grafo {  //Grafo Dirigido
    ...
    public int edges(int u){    //PREGUNTA 2
        if(!isVerticeValido(u, "edges")){
            return -1;
        }
        desmarcarTodos();
        return edges1(u);
    }
    private int edges1(int u){
        marcar(u);
        List<Integer>conectividad = getConectividad(u);
        int cant = V[u].length(); // cantidad de vertice dirigidos del vertice U.
        for(int i=0; i<conectividad.size(); i++){
            int w = conectividad.get(i);
            if(!isMarcado(w)){
                cant+= edges1(w);
            }
        }
    ...
    //METODO ADICIONAL
    private List<Integer>getConectividad(int u){
        List<Integer> conectividad = new ArrayList<Integer>();
        for(int i=0; i<V[u].length(); i++){
            conectividad.add(V[u].get(i));
        }
        //RECORREMOS TODOS LOS VERTICES PARA SABER QUIENES TIENEN COMO ADYACENTE AL VERTICE "u"
        for(int v = 0; v<n; v++){
